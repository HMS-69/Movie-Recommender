...
async def lifespan(app: FastAPI):
    ...
    with open(INDICES_PATH, "rb") as f:
        ...
        app.state.title_to_idx = build_title_to_idx_map(indices_obj)

    # Sanity Check
    if app.state.df is None or "title" not in app.state.df.columns:
        raise RuntimeError("Loaded DataFrame is invalid.")

    yield 
    # --- Shutdown Logic (Optional) ---
    print("Cleaning up resources...")
    del app.state.df
    del app.state.tfidf_matrix

app = FastAPI(lifespan=lifespan)

@app.get("/recommend")
async def recommend(title: str):
    # Access the data via request.app.state
    # df = request.app.state.df
    return {"message": "Data is ready to use!"}
